...
package com.example.inkp.timesnap.crop;
...
import com.example.inkp.timesnap.R;
import com.example.inkp.timesnap.crop.cropwindow.edge.Edge;
import com.example.inkp.timesnap.crop.cropwindow.handle.Handle;
import com.example.inkp.timesnap.crop.util.AspectRatioUtil;
import com.example.inkp.timesnap.crop.util.HandleUtil;
import com.example.inkp.timesnap.crop.util.PaintUtil;
...
public class CropImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    private void personalInitializer() {
        mGuidelinesMode = 1;
        ...
        mHandleRadius = 24 * scale;
        mSnapRadius = 3 * scale;
        mBorderThickness = 3 * scale;
        mCornerThickness = 5 * scale;
        mCornerLength = 20 * scale;
    }
...
//        typedArray.recycle();
    ...
    public Bitmap getCroppedImage() {
        ...
        // T.T .... Temporary solution
        ViewGroup rootView;
        rootView = (RelativeLayout) ((Activity) context)
                .findViewById(R.id.composer_rl_composer_root);
        ...
        if(originalBitmap.getHeight() > originalBitmap.getWidth()){
            int offset = (rootView.getWidth()
                    - Math.round((originalBitmap.getWidth() * scaleX))) / 2;

            cropX = ((bitmapLeft + Edge.LEFT.getCoordinate() - offset) / scaleX);
            cropY = (bitmapTop + Edge.TOP.getCoordinate()) / scaleY;

        } else if(originalBitmap.getHeight() < originalBitmap.getWidth()){
            int offset = (rootView.getHeight()
                    - Math.round((originalBitmap.getHeight() * scaleX))) / 2;

            cropX = (bitmapLeft + Edge.LEFT.getCoordinate()) / scaleX;
            cropY = ((bitmapTop + Edge.TOP.getCoordinate() - offset) / scaleY);
        }
        ...
        Log.d("SELECT_INFO", "x = " + cropX + "\ny = " + cropY + "\nw = " + cropWidth + "\nh = " + cropHeight);
        originalBitmap.setHasAlpha(true);
        for (int i = (int) cropY; i < (int) (cropY + cropHeight); i++) {
            for (int j = (int) cropX; j < (int) (cropX + cropWidth); j++) {
                originalBitmap.setPixel(j, i, Color.alpha(Color.TRANSPARENT));
            }
        }
        Log.d("bitmap__", "processing done");
